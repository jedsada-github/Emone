...
static uint8_t *WiMOD_HCI_ProcessRxMessage (UINT8 *rxData, int rxLength);
...
//------------------------------------------------------------------------------
    ...
    bool
WiMOD_HCI_Init (UART_HandleTypeDef *comPort,
...
WiMOD_HCI_SendMessage (TWiMOD_HCI_Message *txMessage)
{
    ...
    if (!txMessage)
    {
        USART_Transmit (&huart2, "Error: WiMOD_HCI_SendMessage\r\n");
        // error
        return -1;
    }
