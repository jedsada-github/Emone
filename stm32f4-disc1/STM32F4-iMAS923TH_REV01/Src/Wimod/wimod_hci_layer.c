...
int32_t WiMOD_HCI_SendMessage(TWiMOD_HCI_Message *txMessage)
{
    ...
    uint16_t crc16 = CRC16_Calc(&txMessage->SapID, txMessage->Length + WIMOD_HCI_MSG_HEADER_SIZE, CRC16_INIT_VALUE);
    ...
    txMessage->Payload[txMessage->Length + 1U] = HIBYTE(crc16);
    ...
    if (txLength > 0U)
    {
        ...
        if (SerialDevice_SendData(TxBuffer, txLength) > 0U)
        {
            /* Return ok */
...
void WiMOD_HCI_Process()
{
    ...
    rxLength = SerialDevice_ReadData(rxBuf, sizeof(rxBuf));
